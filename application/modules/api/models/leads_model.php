...
class Leads_model extends CI_Model {
	...
	public function leadStat($startDate = '', $endDate = '') {	
		
		if(($startDate && $endDate) != '')
		{
			
			$this->db->select('
			DATE_FORMAT(timestamp, "%Y-%m-%d") AS stamptime,
			COUNT(lead_id) AS countlead
			', FALSE)
				->from('leads')
				->where('DATE_FORMAT(timestamp, "%Y-%m-%d") >=', $startDate)
				->where('DATE_FORMAT(timestamp, "%Y-%m-%d") <=', $endDate)
				->group_by('stamptime')
				->order_by('stamptime', 'asc');
		}
		else 
		{
			$this->db->select('
		...
		if ($query->num_rows() > 0){
			$response['rc']					    = 0;
			$response['success']			    = true;
			$response['data']['leadstat']  = $query->result();
			$response['log_query']			    = str_replace('\n',' ',$this->db->last_query());	
		}
		else{ //no record found	 
			$err_message = ( $this->db->_error_message() ) ? $this->db->_error_message() : 'Lead Stat List: No Records Found.';
			$response['rc']			= 999;
			$response['success']	= false;
			$response['message'][]	= $err_message;
			$response['log_query']	= str_replace('\n',' ',$this->db->last_query());	
