...
class Merchant extends CI_Controller {
	...
	{
		$this->authKey	 	 	 = ( $this->uri->segment(5) ) ? $this->uri->segment(5) : '';
		$this->productId 		 = ( $this->uri->segment(6) ) ? $this->uri->segment(6) : '';
		...
		if ( $is_valid_auth['rc'] == 0 ){
			...
			if ( $this->productId != '' ){
				//get company info
				$response = $this->product_model->creditcardInfo($this->productId);
			}
			else{
				//get company list
				$response = $this->product_model->creditcardList();
			}
		}
		else{
			$response['rc']			= $is_valid_auth['rc'];
		...
		$log_data = array(
			'log_client_id' => $this->authKey,
			'log_method' 	=> 'CREDITCARDS - '.$_SERVER['REQUEST_METHOD'],
			'log_url' 		=> $this->uri->uri_string(),
			'log_request' 	=> json_encode($this->input->post()),
			'log_response' 	=> json_encode($response),
			'log_query' 	=> $response['log_query'],
		...
		$this->apilog_model->apiLog($log_data); //db logs
		$this->api_functions->apiLog(json_encode($log_data),'CREDITCARDS'); //text logs
