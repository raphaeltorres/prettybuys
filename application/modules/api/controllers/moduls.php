...
/**
  * API Controller for Modules
  * What: It's controles all Modules API Functions
 ...
 **/
	...
	public function __construct() {
		...
		$this->moduleId 	= ( $this->uri->segment(4) ) ? $this->uri->segment(4) : '';
	}
	...
	public function get()
	{
		...
		if ( $is_valid_auth['rc'] == 0 ){
			...
			if ( $this->moduleId != '' ){
				//get user info
				$response = $this->modules_model->getModuleinfo($this->moduleId);
			}
			else{
				//get user list
		...
		if ( $is_valid_auth['rc'] == 0 ){
			...
			if ( $this->moduleId != '' ){
				...
				if( !empty($arr_data) ){
					$this->load->model('users_model');
					//edit user info
					$response = $this->users_model->editUser($this->moduleId,$arr_data);						
				}
			}
	...
	public function delete()
	{
		...
		if ( $is_valid_auth['rc'] == 0 ){
			...
			if ( $this->moduleId != '' ){
				...
				$this->load->model('users_model');
				$return = $this->users_model->delUser($this->moduleId);					
			}
			else{ //user id is missing
				$response['rc']			= 999;
				$response['success']	= false;
