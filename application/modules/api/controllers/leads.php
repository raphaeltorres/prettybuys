...
class Leads extends CI_Controller {
	...
	function leadstats()
	{
		...
		$this->startDate    = ( $this->uri->segment(6) ) ? $this->uri->segment(6) : '';
		$this->endDate    	= ( $this->uri->segment(7) ) ? $this->uri->segment(7) : '';
		...
		if ( $is_valid_auth['rc'] == 0 ){
			...
			if ( $this->startDate != '' &&  $this->endDate){
				//get leads info
				$response = $this->leads_model->leadStat($this->startDate,$this->endDate);
			}
			else{
				//get list of leads
				$response = $this->leads_model->leadStat();
			}
		}
		else{
			$response['rc']			= $is_valid_auth['rc'];
		...
		$log_data = array(
			'log_client_id' => $this->authKey,
			'log_method' 	=> 'LEADSSTATS - '.$_SERVER['REQUEST_METHOD'],
			'log_url' 		=> $this->uri->uri_string(),
			'log_request' 	=> json_encode($this->input->post()),
			'log_response' 	=> json_encode($response),
			'log_query' 	=> $response['log_query'],
		...
		$this->apilog_model->apiLog($log_data); //db logs
		$this->api_functions->apiLog(json_encode($log_data),'GET_LEADSTAT'); //text logs
